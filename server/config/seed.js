...
export default function seedDatabaseIfNeeded() {
    ...
    let User = sqldb.User;
    let SurveyIdGenerator = sqldb.SurveyIdGenerator;
    let promises = [];
    ...
    let surveyIdPromise = SurveyIdGenerator.destroy({where: {}})
        .then(() => SurveyIdGenerator.bulkCreate(
            [{
                surveyId: '123456',
                clientId: 0,
                createdBy: 0,
        ...
        .then(() => console.log('finished populating surveyId'))
        .catch(err => console.log('error populating things', err));
    promises.push(surveyIdPromise);
